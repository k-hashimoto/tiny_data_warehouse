...
pub async fn save_table_meta(
    ...
    // DuckDB コメントを更新
    db.set_table_comment(schema_name.clone(), table_name.clone(), comment).await?;
    ...
    // updated_at を更新
    let _ = db.touch_table_timestamp(schema_name.clone(), table_name.clone(), "adhoc".to_string(), false).await;
